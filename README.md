...
> 웹 자료조사부터 보고서 초안 작성, 근거 정합성 검증, 작성 중 능동형 보조까지 — 데이터를 외부로 보내지 않고 내 PC 안에서 끝냅니다.

VERITAS는 *Veritas(라틴어로 "진실")* 라는 이름 그대로, **수집한 근거에 끝까지 책임지는 글쓰기**를 목표로 합니다. 클라우드 LLM API 없이 로컬 GGUF 모델(llama.cpp)만으로 동작하므로 API 키도, 데이터 유출 걱정도 없습니다.
...
- **로컬 우선(Local-first)** — 모든 LLM 추론·임베딩·검색·저장이 사용자 PC에서 실행. 외부로 나가는 건 자료조사를 위한 웹 검색뿐.
- **데스크톱 앱** — PySide6(Qt6) 기반 GUI. 백엔드는 FastAPI, 둘은 HTTP로 통신.
...
| ✅ **정합성 검증 (Verify)** | 보고서를 **임베딩 + IR/NLP 알고리즘**(BM25·RRF·커뮤니티 탐지·PageRank)과 **LLM 신뢰도 판정**으로 재검증. 섹션별 근거, 출처 간 합의/충돌, 문서별 신뢰도(높음/중간/낮음)를 산출합니다. |
| 📝 **초안 작성 (Draft)** | 워크스페이스 지식베이스를 근거로, 선택한 **양식·목차·톤**에 맞춰 실제 문서(주간 보고, 회의록, 사업 제안서 등)를 생성. 기존 양식 파일(.docx/.hwp/.pdf)에서 구조만 추출해 템플릿으로 재사용 가능. |
| 🪄 **AI 보조창 + 화면 모니터링** | 떠 있는 보조 창이 활성 윈도우(워드·파워포인트·에디터)의 텍스트를 OCR/UI Automation으로 읽어, 작성 맥락을 감지하고 **먼저 제안**합니다. (Windows 전용) |
...
| 📊 **대시보드 · 설정** | 워크스페이스·문서 통계, 모델 라이브 전환, 로컬 폴더 접근 및 문서 작업 도구 등록. |
...
| 영역 | 사용 기술 |
|---|---|
| **LLM 추론** | [llama.cpp](https://github.com/ggml-org/llama.cpp) `llama-server` (OpenAI 호환 API) · GGUF 양자화 모델 |
...
| **문서 입출력** | pypdf · python-docx · python-pptx · olefile(HWP) · markdown · [pypandoc](https://pypi.org/project/pypandoc/) |
...
도메인 서비스         services/             RAG · 산출물 저장 · 검증 · 화면 캡처 (상태/로직 소유)
...
각 단계는 `tools/`의 tool을 `ToolRegistry`로 호출하고, 진행 이벤트를 콜백으로 흘려보내 프론트엔드가 실시간 폴링으로 렌더링합니다.
...
veritas/
├─ main.py                  # CLI 진입점
├─ launcher.py              # 통합 런처 (= veritas-launcher.exe 대상)
...
│  ├─ screen_tool_funcs/    #   화면 OCR/UIA 캡처 · 능동형 개입 감지
│  └─ verification/         #   정합성 검증 (sections · reliability · consensus)
...
│  │                        #     document_assist, write, workspaces, dashboard …)
...
└─ runs/<workspace>/        # 워크스페이스별 산출물 (corpus·summary·chromadb·final.md …)
```
...
1. **첫 실행 시** 모델 설정 창을 띄워 Qwen3.5 GGUF 모델을 선택받고, 없는 모델은 Hugging Face에서 진행률과 함께 다운로드합니다.
2. **`llama-server` 2개**를 기동 — 채팅(`8080`) · 임베딩(`8081`, `--embeddings`).
3. **FastAPI 서버**(`8000`)를 기동.
4. **PySide6 데스크톱 UI**를 띄웁니다.

런처가 죽으면(정상 종료·창 닫기·작업 관리자 강제 종료 포함) Windows Job Object가 모든 자식 프로세스를 함께 정리하므로, 포트를 잡고 남는 좀비 `llama-server`가 생기지 않습니다.
...
| 벡터 인덱스 | `runs/<workspace>/chromadb/` | RAG용 임베딩 |
