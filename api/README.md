...
내부적으로 `runs/` 하위 폴더를 스캔해 `final.md` / `summary/index.json` / `summary/doc_*.md` / `summary/request.txt` 중 하나라도 있는 디렉토리를 워크스페이스로 인정. `request.txt`만 있는 경우(조사가 막 시작되어 아직 final이 없는 상태)는 `status: "running"`으로 표시되어 사이드바 드롭다운에 즉시 나타남. `default` (= `runs/api/`) 는 실제 워크스페이스가 하나라도 있으면 목록에서 제외됨.
...
| `workspace_created` | term grounding 직후 새 `runs/<id>/` 폴더가 reserve되고 `summary/request.txt`가 기록된 직후 | `workspaceId`, `name`, `path` |
...
채팅은 두 가지 진입점이 있고 **동일한 워크스페이스 chat history**(`<workspace>/chat_history.json`)를 공유합니다 — 메인 채팅 페이지와 떠 있는 보조 창이 같은 대화를 봅니다.
...
`done` 후 백엔드는 `(user, assistant)` 한 턴을 워크스페이스 `chat_history.json`에 자동 persist.
